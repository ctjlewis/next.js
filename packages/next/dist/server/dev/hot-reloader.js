        ...
        for (const config of configs){
            ...
            config.entry = async (...args)=>{
                ...
                await Promise.all(Object.keys(_onDemandEntryHandler.entries).map(async (pageKey)=>{
                    ...
                    const { bundlePath , absolutePagePath  } = _onDemandEntryHandler.entries[pageKey];
                    ...
                    entrypoints[bundlePath] = (0, _entries).finalizeEntrypoint(bundlePath, isClientCompilation ? `next-client-pages-loader?${(0, _querystring).stringify(pageLoaderOpts)}!` : absolutePagePath, !isClientCompilation, _webpack.isWebpack5);
                }));
                return entrypoints;
            };
        }
