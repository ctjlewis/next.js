...
class BuildStatsPlugin {
    ...
    apply(compiler) {
        compiler.hooks.done.tapAsync('NextJsBuildStats', async (stats, callback)=>{
            const compilationSpan = _profilingPlugin.spans.get(stats.compilation) || _profilingPlugin.spans.get(compiler);
            try {
                ...
                await writeStatsSpan.traceAsyncFn(()=>{
                    return new Promise((resolve, reject)=>{
                        ...
                        const statsJson = reduceSize(_webpack.isWebpack5 ? stats.toJson({
                            ...baseOptions,
